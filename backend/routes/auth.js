  ...
  users.findOne({ username: req.body.username }, (err, user) => {
    ...
    if (user) {
      ...
      if (req.body.username.length < 6 || req.body.username.length > 32) {
        return res.status(400).json({ success: false, message: "Username must be between 6 and 32 characters in length" });
      }
      if (req.body.password.length < 6 || req.body.password.length > 32) {
        return res.status(400).json({ success: false, message: "Password must be between 6 and 32 characters in length" });
      }
    }
    // if (req.body.username.length < 6 || req.body.username.length > 32) {
    //     return res.status(400).json({
